...
export class AdminLoginUsecase implements IAdminLoginUsecase {
  ...
  async execute(dto: loginUserInputDTO): Promise<loginOutputDTO> {
    ...
    const isPasswordValid = await this._hashService.compare(dto.password, user.password);
    ...
    const { token: refreshToken } = this._tokenService.generateRefreshToken(payload);
